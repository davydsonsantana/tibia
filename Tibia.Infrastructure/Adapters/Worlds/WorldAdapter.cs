    ...
    public class WorldAdapter : IWorldAdapter {
        ...
        public async Task<List<World>> List() {
            ...
            for (int i = 1; i < trWorlds.Count; i++) {
                ...
                var battlEye = GetBattlEye(tdCollection[4]);
                var additionalInfo = tdCollection[5].Text;

                var world = new World(worldName, location, pvpType, battlEye, additionalInfo);
                worldList.Add(world);
            }
        ...
        private EBattlEye GetBattlEye(IWebElement tdBattleEye) {
            try {
                ...
                if (battleEyeImg.Contains("icon_battleyeinitial.gif")) {
                    return EBattlEye.ProtectedInicial;
                } else if (battleEyeImg.Contains("icon_battleyeinitial.gif")) {
                    return EBattlEye.Protected;
                } else {
                    return EBattlEye.Unprotected;
                }
